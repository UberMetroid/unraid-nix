...
pub fn write_config_and_metadata(
    ...
    if !env_vars_json.is_empty() {
        if let Ok(serde_json::Value::Object(map)) =
            serde_json::from_str::<serde_json::Value>(env_vars_json)
        {
            for (k, v) in map {
    ...
    let env_opt = if env_list.is_empty() {
        None
    } else {
        Some(env_list)
    };
    ...
    if let Err(e) = config::save_config(&cfg, PROCESS_COMPOSE_CONFIG) {
        crate::store::log_event(
            "ERROR",
            &format!("install-service: failed to save process-compose config for '{name}': {e}"),
        );
        exit(1);
    }
    ...
    if let Err(e) = std::fs::create_dir_all(METADATA_DIR) {
        crate::store::log_event(
            "ERROR",
            &format!("Failed to create metadata dir '{METADATA_DIR}': {e}"),
        );
        exit(1);
    }
    let body = match serde_json::to_string_pretty(&metadata) {
        ...
        Err(e) => {
            crate::store::log_event(
                "ERROR",
                &format!("Failed to serialize metadata for '{name}': {e}"),
            );
            exit(1);
        }
    };
    if let Err(e) = std::fs::write(format!("{METADATA_DIR}/{name}.json"), body) {
        crate::store::log_event(
            "ERROR",
            &format!("Failed to write metadata file for '{name}': {e}"),
        );
        exit(1);
    }
    ...
    if let Err(e) = crate::cli::supervisor::restart_nix_supervisor() {
        crate::store::log_event(
            "ERROR",
            &format!("Failed to restart supervisor after installing service '{name}': {e}"),
        );
        exit(1);
    }
    crate::store::log_event(
        "INFO",
        &format!("Service '{name}' installed/updated successfully. URI: {uri}"),
    );
