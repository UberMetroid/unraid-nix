...
pub fn save_settings(args: &crate::cli::args::SaveSettingsArgs) {
    ...
    let enable_sandbox = args
        .enable_sandbox
        .clone()
        .unwrap_or_else(|| "yes".to_string());
    let show_in_nav = args
        .show_in_nav
        .clone()
        .unwrap_or_else(|| "yes".to_string());
    let allow_source_builds = args
        .allow_source_builds
        .clone()
        .unwrap_or_else(|| "no".to_string());
    let filter_presets_by_hardware = args
        .filter_presets_by_hardware
        .clone()
        .unwrap_or_else(|| "yes".to_string());
    let enable_pid_isolation = args
        .enable_pid_isolation
        .clone()
        .unwrap_or_else(|| "yes".to_string());
    let enable_uts_isolation = args
        .enable_uts_isolation
        .clone()
        .unwrap_or_else(|| "yes".to_string());
    let enable_ipc_isolation = args
    ...
    if !clean_store_path.is_empty() && clean_store_path != clean_old_store_path {
        migration_performed =
            migration::migrate_nix_store(&clean_old_store_path, &clean_store_path);
    }
    ...
    if !default_appdata_path.is_empty() {
        cfg_content.push_str(&format!(
            "DEFAULT_APPDATA_PATH=\"{default_appdata_path}\"\n"
        ));
    }
    if let Err(e) = std::fs::write(NIX_CFG_PATH, cfg_content) {
        crate::store::log_event(
            "ERROR",
            &format!("Failed to write nix.cfg to flash drive at {NIX_CFG_PATH}: {e}"),
        );
        exit(1);
    }
...
pub fn validate_settings(store_path: &str, default_appdata_path: &str) -> Result<(), String> {
    crate::store::validate_store_path(store_path)
        .map_err(|e| format!("invalid store path '{store_path}': {e}"))?;
    if !default_appdata_path.is_empty() && default_appdata_path.starts_with("/boot") {
        return Err(
            "Default Appdata Path cannot be located on the boot flash drive (/boot).".to_string(),
        );
    }
    Ok(())
}
