...
use crate::api::utils::{extract_package_uri, get_service_web_port, HostAddr};
...
use super::cells::{render_autostart_cell, render_lan_ip_port_cell, render_resources_cell};
...
pub fn render_service_row(
    ...
    let is_stopped =
        status_lower == "stopped" || status_lower == "completed" || status_lower == "terminating";
    ...
    let lan_ip_port_html =
        render_lan_ip_port_cell(port_num, is_running, host_ips, &bind_address_override);
    ...
    let start_btn = if !is_running {
        format!(
            r#"<button type="button" class="nix-btn nix-btn-sm" onclick="serviceAction('{}', 'start')" title="Start"><i class="fa fa-play" style="color: #2ecc71;"></i></button>"#,
    ...
    let stop_btn = if is_running {
        format!(
            r#"<button type="button" class="nix-btn nix-btn-sm" onclick="serviceAction('{}', 'stop')" title="Stop"><i class="fa fa-stop" style="color: #e74c3c;"></i></button>"#,
            html_escape(&js_escape(&s.name))
        )
    } else {
