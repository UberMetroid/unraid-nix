...
pub fn get_service_icon_path(name: &str) -> Option<String> {
    ...
    if let Some(idx) = command.find("/nix/store/") {
        ...
        if parts.len() >= 4 {
            ...
            if !candidate.contains("-bash-")
                && !candidate.contains("-bash-interactive-")
    ...
    if let Ok(entries) = std::fs::read_dir("/nix/store") {
        ...
        candidates.sort_by_key(|path| {
            let name_str = path
                ...
                .to_string_lossy()
                .to_lowercase();
            let has_extra = name_str.contains("-web")
                || name_str.contains("-ffmpeg")
                || name_str.contains("-data")
                || name_str.contains("-bin")
                || name_str.contains("-lib");
            if has_extra {
                1
            } else {
                0
...
fn find_image_in_dir(dir: &std::path::Path, root: &std::path::Path) -> Option<String> {
    ...
    for c in &candidates {
        if let Some(filename) = c.file_name().and_then(|f| f.to_str()) {
            ...
            if name_lower == "logo.svg"
                || name_lower == "logo.png"
                || name_lower.contains("jellyfin.svg")
            {
    ...
    for c in &candidates {
        if let Some(filename) = c.file_name().and_then(|f| f.to_str()) {
            let name_lower = filename.to_lowercase();
            if name_lower.contains("logo")
                || name_lower.contains("icon")
                || name_lower.contains("favicon")
            {
                return Some(c.to_string_lossy().to_string());
            }
        }
